<div>
        ...
        <div x-show="active === {{ $index }}"
            ...
            <div class="relative w-full max-w-[1800px] mx-auto px-6 pt-32 h-full flex flex-col justify-end pb-16 md:pb-24 z-10">
                ...
                <div class="flex items-center gap-2"
                    ...
                    <span class="bg-red-600 text-white text-[10px] font-extrabold px-2.5 py-1 rounded uppercase tracking-wider font-semibold">{{ isset($heroMovie['type']) ? str_replace('_', ' ', $heroMovie['type']) : 'SERIES' }}</span>
                </div>
    ...
    <div class="flex items-center justify-between border-b border-zinc-800 pb-4">
        ...
        <div class="text-xs text-zinc-400 bg-zinc-900 px-2.5 py-1 rounded border border-zinc-800">
            Total Loaded: <span class="text-red-500 font-bold">{{ count($series) }}</span> TV Series
        </div>
    </div>
    ...
    @if(empty($series))
        <div class="flex flex-col items-center justify-center py-16 text-zinc-500 bg-zinc-900/50 rounded-xl border border-dashed border-zinc-800">
            ...
            <p class="text-sm font-medium">Tidak ada TV series yang ditemukan.</p>
        </div>
    @else
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            @foreach($series as $item)
                @php
                    $detailUrl = !empty($item['slug']) ? \App\Filament\Streaming\Pages\ShowSeries::getUrl(['slug' => $item['slug']]) : '#';
                @endphp
        ...
        @if($hasMorePages)
            <div x-intersect="$wire.loadMore()" class="flex items-center justify-center pt-8 mt-6">
                <div class="flex items-center gap-3 text-zinc-400 bg-zinc-900/50 px-5 py-2.5 rounded-full border border-zinc-800 shadow-sm">
                    ...
                    <span class="text-sm font-semibold tracking-wide">Loading more TV series...</span>
                </div>
            </div>
        @endif
    @endif
